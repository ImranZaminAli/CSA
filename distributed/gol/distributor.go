...
func makeCall(client rpc.Client, p Params, world [][]byte) *stubs.Response{
	req := stubs.Request{
		World:       world,
		Turns:       p.Turns,
		Threads:     p.Threads,
	...
	client.Call(stubs.ProcessTurnsHandler, req, res)
...
loop:
	...
	c.events <- FinalTurnComplete{res.Turns, getAliveCells(p, res.World)}
